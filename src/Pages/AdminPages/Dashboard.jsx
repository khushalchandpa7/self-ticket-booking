  ...
  const loadEvents = async () => {
    try {
      ...
      if (res && res.data) {
        ...
        const bCap = (events || []).map((e) => ({
          name: e.title,
          y: e.capacity || 0,
        }));
        const cCount = (events || []).map((e) => ({
          name: e.title,
          y: e.cancelledCount || 0,
        }));
        const bRev = (events || []).map((e) => ({
          name: e.title,
          y: e.bookingRevenue || 0,
        }));

        setBookingCount(bCount);
        setBookingCapacity(bCap);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {dashBoardStatasctics.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`${item.bg} rounded-2xl shadow-sm p-5 flex justify-between items-center transition hover:shadow-md`}
            >
              <div className="w-full">
                <div className="flex w-full justify-between items-start">
                  <div
                    className={`w-10 h-10 ${item.iconBg} rounded-xl flex items-center justify-center mb-2`}
                  >
